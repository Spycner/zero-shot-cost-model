...
import features
from enums import FeatureType, Operator, DatabaseSystem


#! TODO: Script is in dire need of refactoring
...
def postgres_plan_collator(
    ...
    plan_featurization = features.__dict__[plan_featurization_name]
...
def create_node_types_per_depth(plan_depths, plan_to_plan_edges):
    ...
    for u, v in plan_to_plan_edges:
        ...
        v_node_id, d_v = plan_dict[v]
        assert d_v < d_u
        data_dict[(f"plan{d_u}", "intra_plan", f"plan{d_v}")].append(
            (u_node_id, v_node_id)
        )
